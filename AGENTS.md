...
**Tests**: `php artisan test` (single test: `php artisan test --filter=testName`)
**PHP Format**: `vendor/bin/pint --dirty`
**JS Lint**: `npm run lint`
**JS Format**: `npm run format`
**Build**: `npm run build`
**Dev**: `composer run dev` (includes server, queue, logs, vite)
...
- Use Eloquent relationships over raw queries; avoid `DB::` queries
- Create Form Request classes for validation; use `php artisan make:` commands
- Write tests using Pest: `php artisan make:test --pest <name>`

## Code Style - Frontend

- Use Inertia v2 with React 19; components in `resources/js/Pages`
- Use `useForm` helper for forms; `<Link>` for navigation
...
- Test all changes: write/update tests, run affected tests first
...
- `fix: resolve memory leak in chat streaming`
- `chore: remove unused DateTimeFormatter class`

## Current System Status (Jan 2026)

### ✅ Complete MCP DateTime Integration

- Dynamic MCP tool discovery via `/tools/list` endpoint
- LLM-powered tool selection (OpenAI & Bedrock)
- Provider injection architecture (fixes critical bug)
- Indonesian language support for datetime responses
- Zero manual configuration needed

### 🏗️ Architecture Flow

```
User Query → LLM Classification → LLM Tool Selection → MCP Tool Execution → LLM Response
```

### 🔧 Key Files

- `app/Services/ToolCoordinator.php` - LLM tool selection & execution
- `app/Services/McpClient.php` - MCP server communication
- `app/Services/LLM/Providers/` - OpenAI & Bedrock with tool support
- `tests/Feature/ToolProviderInjectionTest.php` - Provider injection tests

### 🎯 Provider Injection Architecture

- OpenAI chats → OpenAIProvider → OpenAI API for tool selection
- Bedrock chats → BedrockProvider → Bedrock API for tool selection
- Perfect provider consistency throughout system
